...
function append_Poly_End_tag(key_Tag_Str, polymer_Tag_Str) {
    ...
    if (temp_xaml_html_Obj.hasOwnProperty(key_Tag_Str)) {
        ...
        //  console.log('post'+start_Tag_Str)
    }
    end_Stack.push(polymer_cui_end)
    //console.log('****' + customElement_Stack)

    //console.log(polymer_cui_end)
    return polymer_Tag_Str
}
...
function generate_CUI(element_Obj, xaml_Tag_Stack, Seq_Stack) {
    ...
    seq: for (key_XAML_Model in element_Obj) {
        ...
        //  it will execute for all siblings
        ...
        if (key_XAML_Model == sequence) {
            ...
            XAML: for (key_tag in XAML_Obj) {
                ...
                if (child_Obj) {
                    for (key_Child in child_Obj) {
                        ...
                        if (isEmpty(child_Obj) != true) {
                            ...
                            //  console.log('childobj'+startStr)
                            generate_CUI(child_Obj, xaml_Tag_Stack, Seq_Stack)
                            //  console.log('neha'+startStr)
                        }
                    }

                }
                //  console.log('PREEndtag'+startStr)
                //  remove obj that has been closed from stack_Obj
                ...
                if (temp_xaml_html_Obj.hasOwnProperty(endPop)) {
                    ...
                    //  console.log('POSTEndtag'+startStr)
                    //  console.log(key_tag)
                    //  pop the last element out of the array
                    //  sequence++
                    Seq_Stack.pop()
                    //  console.log(sequence+'postinc')
                    //  console.log(just_popped+'pop')
                    //  console.log('POSTEndtag'+startStr)
                    //  var temp_obj=stack_Obj.pop()
                    //  console.log(temp_obj)
                }
                //  console.log('POSPOST'+startStr)
            }
        }
        else {
            console.log('here')
    ...
    //  console.log('prereturn'+startStr)
    //  console.log(polymerStr)  
    //  return startStr
}
//  console.log(generate_CUI(cui_Obj, XAML_Stack, Sequence_Stack))
...
fs.writeFileSync("./OUTPUT/CUI_Model_HTML.html", cui_html.splice((append_at_html_index + 10), 0, startStr))
...
fs.writeFileSync("./OUTPUT/CUI_Model_Polymer_HTML.html", polymer_cui_html)
