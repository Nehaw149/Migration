...
function assign_Events() {
    ...
    event_DOMLoaded = "document.addEventListener('DOMContentLoaded', function() {"+ assign_Style() +"});"
    finalEvents = "{\n" + event_DOMLoaded + "\n\n" + finalEvents + "\n}"
    return finalEvents
}
...
var filePath_Polymer_CUI_JS = "../Models/CUI/Polymer/FUI_JS.js"
...
function assign_Style() {
    ...
    fs.writeFileSync("../Models/CUI/Polymer/FUI_CSS.css", value_Style)

    return finalAttributes
}

function write_DOM_CSS(id, style_Obj) {
    var findElement = '', addStyle = '', flag_Style = 0, addAttribute = '', comma_sep_Val = ''
        ...
        for (var key_Temp_Attribute in template_Style.STYLE.ATTRIBUTES) {
            if (key_attributes === key_Temp_Attribute) {
                ...
                if (attribute != "NONE" && value != 0) {
                    ...
                    addAttribute = findElement + ".setAttribute('" + attribute + "', '" + value + "');"
    ...
    if (flag_Style > 0) {
        value_Style = value_Style.concat(findElement + addStyle + "}\n")
    }
}
